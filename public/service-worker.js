...
var CACHE_NAME = "vistoria-cache-v-2";
var urlsToCache = [
    ...
    event.respondWith(
        caches.match(event.request).then((response) => {
            if (response) {
                ...
                return response || requestBackend(event);
            }
...
// Update a service worker
self.addEventListener('activate', (event) => {
    var cacheWhitelist = [CACHE_NAME]; // Add current cache name to whitelist
            ...
            return Promise.all(
                cacheNames.map((cacheName) => {
                    if (cacheWhitelist.indexOf(cacheName) === -1) {
                        return caches.delete(cacheName); // Delete if not in whitelist
                    }
                })
            );
        })
